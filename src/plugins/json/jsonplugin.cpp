/*
 * JSON Tiled Plugin
 * Copyright 2011, Porf铆rio Jos茅 Pereira Ribeiro <porfirioribeiro@gmail.com>
...
bool JsonMapFormat::supportsFile(const QString &fileName) const
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        // This is a good indication, but not present in older map files这是一个很好的指示，但在旧的地图文件中不存在
        if (object.value(QLatin1String("type")).toString() == QLatin1String("map"))
            return true;

        // Guess based on expected property：基于预期属性的猜测
