    ...
    , mMapObjectModel(new MapObjectModel(this))
{
    ...
    // Forward signals emitted from the layer model 将layermodel信号转发到MapDocument
...
bool MapDocument::save(const QString &fileName, QString *error)
{
    ...
    if (!mapFormat->write(map(), fileName)) {//保存文件的function map(){mMap}.get()最开始用MapDocumentPtr::create()创建的地图
        if (error)
    ...
    mLastSaved = QFileInfo(fileName).lastModified();//Returns the date and local time when the file was last modified.
                                 ...
                                 QString *error)
{
    ...
    //将新加入文件名和共享指针绑定起来
    MapDocumentPtr document = MapDocumentPtr::create(map, fileName);//应该这里将文档添加到model中
...
/**
 * Returns the name with which to display this map.
 * It is the file name without its path, or 'untitled.tmx' when the map has no file name.
 */
 ...
 * non-empty area of overlap, but we should also consider overlap with empty area as intersection.
 *定制相交检测是必要的，应为相交是一个非空区域重叠，但我们也应该考虑与空白区域重叠为交集。
...
/**在层堆栈的顶部添加一个给定类型的层,添加新的图层之后会发射编辑图层名字的请求
...
/**将给定的层解组,如果图层本身是一个图层组，那么这个组是不不解析的。如果图层的一部分是这个组，他将被移除在这个组
...
/**
  ...
  * the layers will be shown.否则图层将显示出来
  */
 ...
 */
void MapDocument::unifyTilesets(Map *map)//统一瓦砖集
{
...
{
    switch (mMap->orientation()) {//这个mMap就是通过MapDocumentPtr::create()传递过来的
    case Map::Isometric:
        mRenderer.reset(new IsometricRenderer(mMap.get()));//http://www.cplusplus.com/reference/memory/unique_ptr/get/
        //mRender 重新设置为 (new int)指针
        break;
    case Map::Staggered:
