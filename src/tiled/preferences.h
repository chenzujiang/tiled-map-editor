...
/**这个类提供了用户的首选项,并提供了一个方便访问他们的接口
 * This class holds user preferences参数选择 and provides a convenient interface to
 * access them.
 */
...
public:
    ...
    bool snapToPixels() const { return mSnapToPixels; }//像素对齐
    QColor gridColor() const { return mGridColor; }
    int gridFine() const { return mGridFine; }
    qreal objectLineWidth() const { return mObjectLineWidth; }

    bool highlightCurrentLayer() const { return mHighlightCurrentLayer; }//当前层为高亮
    bool showTilesetGrid() const { return mShowTilesetGrid; }

    enum ObjectLabelVisiblity {
        NoObjectLabels,         //object label hiden
        SelectedObjectLabels,   //object label selected show
        AllObjectLabels         // all object label show
    };
    ...
    /**提供对QSettings实例的访问，允许存储/检索任意值。组和键的命名样式是CamelCase。
