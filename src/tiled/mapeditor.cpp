/*
 * mapeditor.cpp
 * Copyright 2016, Thorbj酶rn Lindeijer <bjorn@lindijer.nl>
 *
...
/**
 * A proxy model that makes sure no items are checked or checkable and that there is only one column.
 *
 *一种代理模型，确保没有items勾选(检查)或可勾选中，并且只有一列。
 *在图层组合框中使用，因为复选框不能在该上下文中使用，但无论如何都在Windows上呈现。
...
MapEditor::MapEditor(QObject *parent)
    : Editor(parent)
    , mMainWindow(new QMainWindow)//mMainWindow这个对象就是在三个对象中的1个
    ...
    , mUndoDock(new UndoDock(mMainWindow))/*history */
    , mObjectsDock(new ObjectsDock(mMainWindow))/*对象dock */
    , mTemplatesDock(new TemplatesDock(mMainWindow))/*模板*/
    , mTilesetDock(new TilesetDock(mMainWindow))/*图块*/
    , mTerrainDock(new TerrainDock(mMainWindow))/*地形*/
    , mWangDock(new WangDock(mMainWindow))/*wangDock */
    , mMiniMapDock(new MiniMapDock(mMainWindow))
    , mLayerComboBox(new TreeViewComboBox)//左下角的comboBox(mapDoument才有),用于激活对应的图层
    , mComboBoxProxyModel(new ComboBoxProxyModel(this))
    , mReversingProxyModel(new ReversingProxyModel(this))
    , mZoomable(nullptr)
    , mZoomComboBox(new QComboBox)
    , mStatusInfoLabel(new QLabel)
    , mMainToolBar(new MainToolBar(mMainWindow))    //主要工具的Action已经在对象中构造
    ...
    , mTileStampManager(new TileStampManager(*mToolManager, this))//？？？
{
    ...
    mMainWindow->setDockOptions(mMainWindow->dockOptions() | QMainWindow::GroupedDragging);//设置窗口的停靠行为
#endif
    mMainWindow->setDockNestingEnabled(true);//dock支持嵌套
    mMainWindow->setCentralWidget(mWidgetStack);

    mToolsToolBar = new QToolBar(mMainWindow);//在主窗口中添加了一个QToolBar（加入各个功能控件）[中]
    ...
    mStampBrush = new StampBrush(this);//图章刷
    mTerrainBrush = new TerrainBrush(this);//地形刷
    mWangBrush = new WangBrush(this);//wang brush
    mBucketFillTool = new BucketFillTool(this);//填充工具
    mEditPolygonTool = new EditPolygonTool(this);//编辑多边形工具
    mShapeFillTool = new ShapeFillTool(this);//Shape Fill Tool形状填充工具
    CreateObjectTool *tileObjectsTool = new CreateTileObjectTool(this);//出入图块
    CreateTemplateTool *templatesTool = new CreateTemplateTool(this);//插入模板
    CreateObjectTool *rectangleObjectsTool = new CreateRectangleObjectTool(this);//插入矩形
    CreateObjectTool *pointObjectsTool = new CreatePointObjectTool(this);//插入点
    CreateObjectTool *ellipseObjectsTool = new CreateEllipseObjectTool(this);//出入椭圆
    CreateObjectTool *polygonObjectsTool = new CreatePolygonObjectTool(this);//插入多边形工具
    CreateObjectTool *textObjectsTool = new CreateTextObjectTool(this);//插入文本
    ...
    mToolsToolBar->addAction(mToolManager->registerTool(new Eraser(this)));//橡皮擦
    mToolsToolBar->addAction(mToolManager->registerTool(new TileSelectionTool(this)));//矩形选择工具
    mToolsToolBar->addAction(mToolManager->registerTool(new MagicWandTool(this)));//魔术棒
    mToolsToolBar->addAction(mToolManager->registerTool(new SelectSameTileTool(this)));//select same tile
    mToolsToolBar->addSeparator();
    mToolsToolBar->addAction(mToolManager->registerTool(new ObjectSelectionTool(this)));//选择对象
    ...
    mToolsToolBar->addSeparator();
    mToolsToolBar->addAction(mToolManager->registerTool(new LayerOffsetTool(this)));//挪动图层
    ...
    mTileStampsDock = new TileStampsDock(mTileStampManager, mMainWindow);//图块图章
    ...
    setSelectedTool(mToolManager->selectedTool());//在构造MapEditor地图编辑器就构造该对象QToolBar
...
/*
 * 给你个widgetStack->产生一个视图->放到场景中->在场景中添加文件数据,场景中就有数据，将在视图中添加场景。在再栈中添加wiew.切换tab实现不同的view
 */
    MapView *view = new MapView(mWidgetStack);//Map编辑者没一个视图的父控件都是相同的
    MapScene *scene = new MapScene(view); // scene is owned by the view

    scene->setMapDocument(mapDocument);
    view->setScene(scene);

    mWidgetForMap.insert(mapDocument, view);//没一个文档都new了一个视图(管理起来)
    mWidgetStack->addWidget(view);//没一个视图的Widgetstack

    // restore the previous state for this map为这张地图恢复以前的状态
    //typedef QMap<QString, QVariant> QVariantMap;
    QVariantMap mapState = mMapStates.value(document->fileName()).toMap();//得到的QVariant.toMap()也是 QMap<QString, QVariant>类型的
    if (!mapState.isEmpty()) {
        ...
        if (Layer *layer = layerAtGlobalIndex(mapDocument->map(), layerIndex))
            mapDocument->setCurrentLayer(layer);//给定图层索引返回指定图层并设置为当前图层
    }
}
...
QList<QDockWidget *> MapEditor::dockWidgets() const
{
    return QList<QDockWidget*> {
        mPropertiesDock,//左边属性添加、删除、从命名对话框
        mLayerDock,     //左上角图层Dock
        mMapsDock,      //左边的有按钮
        mUndoDock,      //history
        mObjectsDock,   //左上角的对象Dock
        mTemplatesDock, //Templates Dock
        mTilesetDock,   //图块 tileset Dock
        mTerrainDock,   //地形Dock
        mWangDock,      //动画编辑的Wang sets
        mMiniMapDock,   //dock add QFrame
        mTileStampsDock //图块图章Dock
    };
}
...
void MapEditor::resetLayout()
{
    ...
    for (auto dockWidget : dockWidgets) {
        mMainWindow->removeDockWidget(dockWidget);//从主窗口布局中删除dockwidget并隐藏它。注意，dockwidget没有被删除。
    ...
    mMainWindow->addToolBar(mMainToolBar);//主工具栏
    mMainWindow->addToolBar(mToolsToolBar);//工具栏(中)
    mMainWindow->addToolBar(mToolSpecificToolBar);//特殊工具栏AbstractObjectTool中给他添加工具
    ...
    // they are hidden by default这些dock小部件对很多人来说可能不是很有用，所以，将他们隐藏
...
void MapEditor::setSelectedTool(AbstractTool *tool)
{
    ...
    if (tool) {
        connect(tool, &AbstractTool::cursorChanged,
                this, &MapEditor::cursorChanged);
//MapEdior传入了QToolBar(给特殊工具栏添加Action)   //StampActions* mStampActions//object
...
{//void LayerOffsetTool::mouseMoved(const QPointF &pos, Qt::KeyboardModifiers modifiers)
    mStatusInfoLabel->setText(statusInfo);//状态信息来自于鼠标移动
}
//图层复合激活
