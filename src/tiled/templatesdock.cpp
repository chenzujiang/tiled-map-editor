    ...
    , mToolManager(new ToolManager(this))
{
    ...
    // Prevent dropping a template into the editing view防止将模板放入编辑视图中
    ...
    // Initially disabled until a change happens初始化是失能的直到发生了改变
    ...
    mDescriptionLabel->setWordWrap(true);//对标签的文字做必要的包装
    mDescriptionLabel->setVisible(true);

    auto toolsLayout = new QHBoxLayout;
    toolsLayout->addWidget(editingToolBar);//QToolBar
    toolsLayout->addWidget(mFixTilesetButton);//button
    ...
    connect(mTemplatesView, &TemplatesView::currentTemplateChanged,this, &TemplatesDock::currentTemplateChanged);
    connect(mTemplatesView, &TemplatesView::currentTemplateChanged,this, &TemplatesDock::setTemplate);
    connect(mTemplatesView, &TemplatesView::focusInEvent,this, &TemplatesDock::focusInEvent);
    connect(mTemplatesView, &TemplatesView::focusOutEvent,this, &TemplatesDock::focusOutEvent);
    connect(mToolManager, &ToolManager::selectedToolChanged,this, &TemplatesDock::setSelectedTool);
