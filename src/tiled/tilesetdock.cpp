    ...
    , mSynchronizingSelection(false)
{
    ...
    connect(mTabBar, &QWidget::customContextMenuRequested,this, &TilesetDock::tabContextMenuRequested);

    QWidget *w = new QWidget(this);

    QHBoxLayout *horizontal = new QHBoxLayout;
    horizontal->setSpacing(0);
    horizontal->addWidget(mTabBar);//最上面一栏 stackwidget TabBar
    horizontal->addWidget(mTilesetMenuButton);//tabbar旁边的下拉按钮

    QVBoxLayout *vertical = new QVBoxLayout(w);
    vertical->setSpacing(0);
    vertical->setMargin(0);
    vertical->addLayout(horizontal);
    vertical->addWidget(mSuperViewStack);//中间那个stackwidget
    ...
    horizontal->setSpacing(0);
    horizontal->addWidget(mToolBar, 1);//最底下那一栏
    vertical->addLayout(horizontal);
