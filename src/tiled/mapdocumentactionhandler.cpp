 ...
 * Copyright 2010-2017, Thorbj酶rn Lindeijer <thorbjorn@lindeijer.nl>
    ...
    , mMapDocument(nullptr)
{
    ...
    mActionSelectInverse = new QAction(this);//反选
    mActionSelectInverse->setShortcut(tr("Ctrl+I"));
    mActionSelectNone = new QAction(this);   //取消选择
    mActionSelectNone->setShortcut(tr("Ctrl+Shift+A"));

    mActionCropToSelection = new QAction(this);//截切选择的部分
    ...
    mActionLayerViaCopy = new QAction(this);//经由,通过拷贝
    ...
    mActionDuplicateLayer->setShortcut(tr("Ctrl+Shift+D"));//复制图层
    ...
    mActionToggleOtherLayers = new QAction(this);//显示/隐藏所有其他图层
    ...
    mActionToggleLockOtherLayers = new QAction(this);//触发,开关
    ...
    mActionDuplicateObjects = new QAction(this);//复制对象
...
/**
 * Creates the new layer menu, which is used in several places.创建新的图层菜单，它在几个地方使用
 */
...
void MapDocumentActionHandler::selectInverse()//Inverse反选
{
    if (!mMapDocument)
        return;
