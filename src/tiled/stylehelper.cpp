...
{
    Q_ASSERT(!mInstance);
    mInstance = new StyleHelper;//[!1]
}
//构造函数
StyleHelper::StyleHelper()
    : mDefaultStyle(QApplication::style()->objectName())//返回应用程序的样式对象并获取名字
    , mDefaultPalette(QApplication::palette())          //返回应用程序的调试板
{
    apply();
...
{
    Preferences *preferences = Preferences::instance();//[!2]

    QString desiredStyle;//期望的样式
