...
MapView::~MapView()
{
    setHandScrolling(false); // Just in case we didn't get a hide event防止我们没得到影藏的事件
}
 ...
 * 将视图集中在给定的场景位置，即使这需要扩展滚动条的范围。
 * 这段代码基于QGraphicsView：：centerOn。
 */
void MapView::forceCenterOn(const QPointF &pos)
{
    ...
    qreal width = viewport()->width();//Returns the viewport widget.
