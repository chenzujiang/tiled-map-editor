...
/**给QShortcut 实例设置动作替代快捷键,使用Parent作为他们的父节点
 * Replaces the shortcuts set on the actions with QShortcut instances, using
 * \a parent as their parent.
 *这样做是为了确保快捷键任然能够使用,当动作仅仅添加到工具栏中，工具栏隐藏
 * 这样做是为了确保即使只将动作添加到工具栏中，并且这个工具栏是隐藏的，也可以使用快捷方式。
...
void ToolManager::createShortcuts(QWidget *parent)
{
    ...
    for (QAction *action : actions) {
        ...
        if (!key.isEmpty()) {
            ...
            // Limit the context of the shortcut to avoid ambiguous overloads限制快捷方式的上下文，以避免模糊的重载
