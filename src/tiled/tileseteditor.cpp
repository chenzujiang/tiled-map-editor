...
TilesetEditor::TilesetEditor(QObject *parent)
    : Editor(parent)
    , mMainWindow(new TilesetEditorWindow(this))//mMainWindow这个对象就是在三个对象中的1个
    ...
    , mPropertiesDock(new PropertiesDock(mMainWindow))//左边属性+-
    , mUndoDock(new UndoDock(mMainWindow))//历史记录            //撤销<empty>
    , mTerrainDock(new TerrainDock(mMainWindow))      //Tileset 地形Dock
    , mTileCollisionDock(new TileCollisionDock(mMainWindow))//图块碰撞编辑器
    , mTemplatesDock(new TemplatesDock(mMainWindow))   //用来装属性
    , mWangDock(new WangDock(mMainWindow))             //wang dock
    , mZoomComboBox(new QComboBox)
    , mStatusInfoLabel(new QLabel)
    , mTileAnimationEditor(new TileAnimationEditor(mMainWindow))//动画编辑框
    ...
    , mCurrentTile(nullptr)
{
    ...
    mTilesetToolBar = mMainWindow->addToolBar(tr("Tileset"));//返回一个QToolBar
    ...
    mMainWindow->statusBar()->addPermanentWidget(mZoomComboBox);//combox
    mMainWindow->statusBar()->addWidget(mStatusInfoLabel);//label [1][-1]
...
QList<QDockWidget *> TilesetEditor::dockWidgets() const
{
    return QList<QDockWidget*> {
        mPropertiesDock,   //属性Dock
        mUndoDock,         //撤销Dock
        mTerrainDock,      //地形Dock
        mTileCollisionDock,//Tile碰撞Dock
        mWangDock          //WangDock
    };
}
