...
/**返回在给定的“作用”下存储的数据，以获得“索引”所引用的条目。
 * Returns the data stored under the given <i>role</i> for the item referred to by the <i>index</i>.
 */
...
void LayerModel::setMapDocument(MapDocument *mapDocument)
{
    ...
    beginResetModel();
    mMapDocument = mapDocument;//MapDocumentPtr::create(map)创建的文档
    mMap = mMapDocument->map();//MapDocumentPtr::create(map)传递的地图
    endResetModel();
}

/**：在这个模型的map中添加一个层，在给定的索引中作为父层的子类插入它。
 * Adds a layer to this model's map, inserting it at as a child of \a parentLayer at the given \a index.
 */
