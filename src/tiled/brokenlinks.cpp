                                    ...
                                    this))
{
    ...
    mDescriptionLabel->setWordWrap(true);//合适的时候词语的换行/包装
    ...
    font.setBold(true);//tile font bold
    mTitleLabel->setFont(font);

    mProxyModel = new QSortFilterProxyModel(this);
    mProxyModel->setSortLocaleAware(true);//当在排序的时候设置本地字符串的感知(有意识地)设置
    mProxyModel->setSortCaseSensitivity(Qt::CaseInsensitive);//此属性保留排序时用于比较字符串的区分大小写设置
    mProxyModel->setSourceModel(mBrokenLinksModel);

    mView->setModel(mProxyModel);
    mView->setRootIsDecorated(false);//tree 展示子项?
    mView->setItemsExpandable(false);//此属性保留用户是否可以交互展开和折叠项
    mView->setUniformRowHeights(true);//应该是设置子项和父项同级的高度？
    ...
    //由于某种原因，模型重置不会触发selectionChanged信号，
    //所以我们需要明确地处理这个问题。
...
void BrokenLinksWidget::tryFixLinks(const QVector<BrokenLink> &links)
{
    ...
    for (const BrokenLink &link : links) {
        if (link.type == TilesetImageSource || link.type == TilesetTileImageSource) {
            if (!editingTileset) {
                // We need to open the tileset document in order to be able to make changes to it...我们需要打开tileset文档，以便能够对它进行更改。
    ...
    // todo: fix caption after string freeze (and the text on the button)待办事项：在字符串冻结之后修复标题（以及按钮上的文本
...
void BrokenLinksWidget::tryFixLink(const BrokenLink &link)
{
    ...
    if (link.type == TilesetImageSource || link.type == TilesetTileImageSource) {
        ...
        // For local images, check if they can be loaded对于本地图像，检查它们是否可以加载
