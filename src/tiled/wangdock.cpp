    ...
    , mInitializing(false)
{
    ...
    connect(mWangSetView->selectionModel(), &QItemSelectionModel::currentRowChanged,this, &WangDock::refreshCurrentWangSet);
    connect(mWangSetView, &QAbstractItemView::pressed,this, &WangDock::indexPressed);

    connect(mProxyModel, &QAbstractItemModel::rowsInserted,this, &WangDock::expandRows);
    ...
    connect(mAddEdgeColor, &QAction::triggered, this, &WangDock::addEdgeColor);
    connect(mAddCornerColor, &QAction::triggered,this, &WangDock::addCornerColor);
    connect(mRemoveColor, &QAction::triggered,this, &WangDock::removeColor);

    mWangTemplateView = new WangTemplateView(w);//Patterns
    mWangTemplateView->setModel(mWangTemplateModel);
    mWangTemplateView->setVisible(true);

    connect(mWangTemplateView->selectionModel(), &QItemSelectionModel::currentChanged,this, &WangDock::refreshCurrentWangId);
    ...
    QVBoxLayout *wangSetVertical = new QVBoxLayout(wangSetWidget);//wangSet view
    wangSetVertical->setMargin(0);
    wangSetVertical->addWidget(mWangSetView);//main one
    wangSetVertical->addLayout(wangSetHorizontal);//main one
    ...
    wangViews->addWidget(wangSetWidget);// main one
    wangViews->addWidget(mTemplateAndColorWidget);//main tow
