 ...
 * Copyright 2009-2015, Thorbj酶rn Lindeijer <thorbjorn@lindeijer.nl>
 *
...
void ExportAsImageDialog::accept()//怎么调用到这个函数 20-10-7
{
    const QString fileName = mUi->fileNameEdit->text();
    if (fileName.isEmpty())
        return;

    if (QFile::exists(fileName)) {//Returns true if the file specified by fileName exists; otherwise returns false.
    ...
    MapRenderer *renderer = mMapDocument->renderer();//return map renderers
...
{
    // Don't confirm overwrite here, since we'll confirm when the user presses the Export button
