    ...
    mChangingLayerOpacity(false)
{
    ...
    opacityLayout->addWidget(mOpacitySlider);//滑动器
    mOpacityLabel->setBuddy(mOpacitySlider);//伙伴控件
    ...
    buttonContainer->addWidget(mNewLayerButton);//新建图层按钮
    ...
    listAndToolBar->addWidget(mLayerView);//对象、图层块等View
    listAndToolBar->addWidget(buttonContainer);//按钮容器

    layout->addLayout(opacityLayout);//透明度
    layout->addLayout(listAndToolBar);//视图  按钮
//layout 是在widget中的辞职布局控件
    setWidget(widget);
    this->retranslateUi();
...
void LayerDock::retranslateUi()
{
    ...
    mOpacityLabel->setText(tr("Opacity:"));//透明度
