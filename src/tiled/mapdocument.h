...
/**代表一个可编辑的地图,这个类的目的是确保任何编辑操作都将导致发出适当的信号,以便使GUI能够相应地进行更新。
 ...
 *map文档提供了层模型，跟踪当前选定的层，并提供了添加和删除map对象的API。它还拥有去QUndoStack。
...
public:
     ...
     * ownership of the map.在给定的地图上构造一个地图文档。地图文档拥有地图的所有权。
     */
     ...
     * 请注意，直接修改map将不允许GUI适当地更新自己
     * Be aware that directly modifying the map will not allow the GUI to update itself appropriately.
     */
...
signals:
    ...
    /**当用户输入编辑地图的某个区域时发出。区域以瓷砖坐标给出。如果编辑了多个图层，将发出多个信号。
...
private:
    ...
    /*使用QPointer是因为这里引用的格式可以由插件动态添加，也可以再次删除。
     * QPointer is used since the formats referenced here may be dynamically added by a plugin, and can also be removed again.
     */
