...
{
    return !mapObject->cell()/*返回与该对象关联的Tiled*/.isEmpty();
}
...
void AbstractObjectTool::deactivate(MapScene *)//使无效
{
    mMapScene = nullptr;
}
...
{
    // Take into account把…视为 the offset of the current layer考虑到当前层的偏移量
...
{
    mFlipHorizontal->setToolTip(tr("Flip Horizontally"));//这个属性包含动作的提示
...
void AbstractObjectTool::duplicateObjects()//复制对象
{
...
void AbstractObjectTool::detachSelectedObjects()//分离所选择的对象
{
...
void AbstractObjectTool::replaceObjectsWithTemplate()//用模板替换对象
{
...
void AbstractObjectTool::flipHorizontally()//水平翻转
{
    mapDocument()->flipSelectedObjects(FlipHorizontally);
}

void AbstractObjectTool::flipVertically()//垂直翻转
{
    mapDocument()->flipSelectedObjects(FlipVertically);
}

void AbstractObjectTool::rotateLeft()//旋转
{
...
void AbstractObjectTool::raise()//上升
{
...
void AbstractObjectTool::raiseToTop()//重下到上
{
    RaiseLowerHelper(mMapScene).raiseToTop();
}

void AbstractObjectTool::lowerToBottom()//下降到底部
{
    RaiseLowerHelper(mMapScene).lowerToBottom();
}

/**显示上下文菜单,这个菜单允许你复制和移动地图对象或者编辑它的属性
