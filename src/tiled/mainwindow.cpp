...
struct ExportDetails//导出的细节
{
    Format *mFormat = nullptr;//格式/版式
    QString mFileName;
...
template <typename Format>//选择导出的细节 全局函数返回是泛型
                                          ...
                                          QWidget* window)
{
    ...
    if (suggestedFilename.isEmpty()) {
        ...
        QString baseName = baseNameInfo.baseName();//返回文件的基本名称,不包括文件路径,包含所有的字符,但是不包括直到第一个点
        //http://www.runoob.com/regexp/regexp-syntax.html
        QRegExp extensionFinder(QLatin1String("\\(\\*\\.([^\\)\\s]*)"));
        //试图Attempts to find a match in str from position offset (0 by default).
    ...
    // No need to confirm overwrite here since it'll be prompted below 不需要在这里确认,以为它将在下面进行提示
    ...
    // If a specific filter was selected, use that format如果指定了特定的过滤器，请使用该格式
    Format *chosenFormat = helper.formatByNameFilter(selectedFilter);

    // If not, try to find the file extension among the name filters如果没有找到，试着在名称过滤器中找到文件扩展名
    ...
    , mDocumentManager(DocumentManager::instance())
{
    ...
    ActionManager::registerAction(mUi->actionNewMap, "file.new_map");//文件菜单下的new/mew_map
    ActionManager::registerAction(mUi->actionNewTileset, "file.new_tileset");

    auto *mapEditor = new MapEditor;//有对应的地图视图栈
    auto *tilesetEditor = new TilesetEditor;//有对应的块视图栈
    ...
    mDocumentManager->setEditor(Document::TilesetDocumentType, tilesetEditor);//栈布局没有显示出来，当没有内容的时候

    setCentralWidget(mDocumentManager->widget());//开始的中心widget(three button)

#ifdef Q_OS_MAC
    MacSupport::addFullscreen(this);
#endif

#if QT_VERSION >= 0x050600//这个属性包含QMainWindow的停靠行为
...
//参数选项
    ...
    tiledIcon.addFile(QLatin1String(":images/32x32/tiled.png"));//给这个QIcon添加图标
     ...
     *If the result contains more than one result,
     *these can be considered alternative shortcuts on the same platform for the given key.对于给定的键，这些可以被认为是同一平台上的另一种快捷方式。
     * 列表的第一个元素表示给定平台的主要快捷方式。如果结果包含多个结果，则可以将这些快捷方式视为给定键在同一平台上的可选快捷方式。
     */
    QList<QKeySequence> deleteKeys = QKeySequence::keyBindings(QKeySequence::Delete);
    qDebug()<<deleteKeys;
    deleteKeys.removeAll(Qt::Key_D | Qt::ControlModifier);  // used as "duplicate" shortcut：作为“复制”快捷方式
#ifdef Q_OS_OSX
    ...
    QList<QKeySequence> redoShortcuts = QKeySequence::keyBindings(QKeySequence::Redo);//获取重置快捷键列表
    qDebug()<<redoShortcuts;
    ...
    mUi->actionSnapToPixels->setActionGroup(snappingGroup);//捕捉Group
    //with checkbox
    ...
    mUi->actionAutoMapWhileDrawing->setChecked(preferences->automappingDrawing());//menuMap
    ...
    mUi->actionLabelForHoveredObject->setChecked(preferences->labelForHoveredObject());//悬停在某个对象时,显示label
    ...
    // Make sure Ctrl+= also works for zooming in//确保Ctrl+=也可以放大
    ...
    mUi->menuEdit->insertAction(mUi->actionCut, undoAction);//在某个动作之前插入一个动作
    ...
    mUi->menuEdit->insertAction(mUi->actionPreferences,
                                mActionHandler->actionSelectAll());//select all
    mUi->menuEdit->insertAction(mUi->actionPreferences,
                                mActionHandler->actionSelectInverse());//inverse select
    mUi->menuEdit->insertAction(mUi->actionPreferences,
                                mActionHandler->actionSelectNone());//select cancel//如何相应插入动作2018-10-7
    mUi->menuEdit->insertSeparator(mUi->actionPreferences);

    mUi->menuMap->insertAction(mUi->actionOffsetMap,
                               mActionHandler->actionCropToSelection());//地图偏移

    mUi->menuMap->insertAction(mUi->actionOffsetMap,
                               mActionHandler->actionAutocrop());

    mLayerMenu = new QMenu(tr("&Layer"), this);//图层菜单不在ui中做的
    ...
    menuBar()->insertMenu(mUi->menuHelp->menuAction(), mLayerMenu);//在帮助菜单前添加图层
    ...
    connect(mUi->actionAddExternalTileset, &QAction::triggered,
            this, &MainWindow::addExternalTileset);//Map menu
    ...
    //https://blog.csdn.net/TemetNosce/article/details/78047481
    connect(mUi->actionLoadWorld, &QAction::triggered, this, [this,preferences]{
        QString lastPath = preferences->lastPath(Preferences::WorldFile);//获取worldFile最后一次打开的位置
    ...
    connect(mUi->menuUnloadWorld, &QMenu::aboutToShow, this, [this] {
        ...
        const auto worldFiles = WorldManager::instance().loadedWorldFiles();
        //给mUi->menuUnloadWorld菜单添加Action(又文件名充当),当点击之后撤销对应的文件(wolrd),并改变mSetting中LoadedWorlds值列表
    ...
    connect(mUi->actionOffsetMap, &QAction::triggered, this, &MainWindow::offsetMap);//挪动地图
    ...
    connect(mUi->actionAutoMapWhileDrawing, &QAction::toggled,
            preferences, &Preferences::setAutomappingDrawing);//自动绘制
    connect(mUi->actionMapProperties, &QAction::triggered,
            this, &MainWindow::editMapProperties);//地图属性

    connect(mUi->actionTilesetProperties, &QAction::triggered,
            this, &MainWindow::editTilesetProperties);//在软件上没有显示
    //help menu
    ...
    mUi->menuUnloadWorld->setEnabled(!WorldManager::instance().worlds().isEmpty());//disbale

    // Add recent file actions to the recent files menu
    for (auto &action : mRecentFiles) {
         action = new QAction(this);
         mUi->menuRecentFiles->insertAction(mUi->actionClearRecentFiles,
                                            action);//在清除Action前面插入一个action
         action->setVisible(false);
         connect(action, &QAction::triggered,
                 this, &MainWindow::openRecentFile);
    }
    mUi->menuRecentFiles->insertSeparator(mUi->actionClearRecentFiles);//加Separator
    ...
    // 添加视图和工具栏子菜单。这需要在dock控件和工具栏被添加到主窗口之后才发生(很多Dock都可以hide and show)
    ...
    mUi->menuView->insertAction(mUi->actionShowGrid, mViewsAndToolbarsAction);//在显示表格Action添加    hide and show dock
    mUi->menuView->insertAction(mUi->actionShowGrid, mShowObjectTypesEditor);//对象类型编辑
    mUi->menuView->insertSeparator(mUi->actionShowGrid);//插入分割线
    ...
    //这个静态函数在给定的时间间隔之后调用一个槽.使用这个函数非常方便，因为您不需要使用timerEvent或者创建一个本地QTimer对象。
}
...
{
    // Play nice with session management and cancel shutdown process when user requests this
    // 一个良好的回话管理窗口,当用户请求时,取消关机进程
    if (manager.allowsInteraction())//请求会话管理器获得与用户交互的权限。如果允许交互，则返回true;否则返回false。
        if (!confirmAllSave())
            manager.cancel();//告诉会话管理器取消关机进程。应用程序不应该在没有询问用户之前调用此函数
}
...
{
    if (confirmAllSave()) {//确保用户不会在下次启动时处于Clear视图模式
...
{
    if (event->key() == Qt::Key_Space && !event->isAutoRepeat())//如果此事件来自自动重复键，则返回true;如果来自初始按键，则返回false。
...
{
    const auto urls = e->mimeData()->urls();//返回MIME数据对象中包含的url列表。url对应于MIME类型text/uri-list
...
void MainWindow::newMap()
{
    ...
    //MapDocumentPtr is created and rev map pointer to Document Constructs
    auto mapDocument = newMapDialog.createMap();

    if (!mapDocument)//MapDocumentPtr
        return;

    if (!mDocumentManager->saveDocumentAs(mapDocument.data()))//Returns the value of the pointer referenced by this object(MapDocment).返回来的值是这个对象的指针的引用
        return;

    mDocumentManager->addDocument(mapDocument);//添加一个新的文档到文档管理者中(ptr)
}
...
bool MainWindow::openFile(const QString &fileName, FileFormat *fileFormat)
{
    ...
    // Select existing document if this file is already open          (如果文档是打开的，就选择这个文档，用在tabbar切换中)
...
void MainWindow::openLastFiles()
{
    ...
    if (openCountVariant.isValid()) {
        const QStringList recentFiles = mSettings.value(
                    QLatin1String("fileNames")).toStringList();//文件名称列表
...
//    "Tiled map files (*.tmx *.xml)"
//    "全部文件 (*);;Tiled map files (*.tmx *.xml)"
//    "Tiled tileset files (*.tsx *.xml)"
//    "全部文件 (*);;Tiled map files (*.tmx *.xml);;Tiled tileset files (*.tsx *.xml)"
/*为FileFormat为基类的对象并有读特性from PluginManager::QbjectList得到该该格式的对象
 *通过该对象获取该对象(+之前都支持的+)所支持的文件格式(第一次打印)。
 *+之前对象所支持的格式文件(第二次打印)
 *将从插件管理者中获取的该对象s放到FileFormat类型的列表中，最后以该对象所有支持文件类型为Key,对象为Value插入Map中
 * Map<QString,Format*> mFomatBynameFilter
 */
    ...
    const auto fileNames = QFileDialog::getOpenFileNames(this, tr("Open File"),
                                                         ...
                                                         helper.filter(),//支持很多的只要是FileFormat为基类的对象文件格式都行
                                                         &selectedFilter);
    if (fileNames.isEmpty())
        return;
    //选择最后打开的格式
    //当选择一个特定的过滤器时，使用相关的格式
    ...
    for (const QString &fileName : fileNames)//循环打开文件(getOpenFileNames)
...
bool MainWindow::saveFile()
{
    ...
    if (currentFileName.isEmpty())//保存文档由用户所设定(规定)的文件名
...
void MainWindow::export_()
{
    ...
    // fall back when no successful export happened当没有成功的export时，就退回去
    exportAs();
}
...
{
    // todo: asking to save is not appropriate here在这里请求保存是不合适的
...
void MainWindow::openPreferences()//打开参数选择
{
...
//menu->newTileset item check button slot funtion
bool MainWindow::newTileset(const QString &path)
{
    Preferences *prefs = Preferences::instance();

    const QString startLocation = path.isEmpty()//null
            ? QFileInfo(prefs->lastPath(Preferences::ImageFile)).absolutePath()//path = mString->value(LastPaths/Images).absolutePath()
            : path;

    NewTilesetDialog newTileset(this);//创建tileset dialog
    ...
    if (mapDocument && newTileset.isEmbedded()) {
        ...
        // Save new external tileset and open it：保存新的外部tileset并打开它
...
void MainWindow::ensureHasBorderInFullScreen()
{
    ...
    QWindow *window = windowHandle();//If this is a native widget, return the associated QWindow. Otherwise return null.
    if (!window)
        return;

    bool wasFullScreen = isFullScreen();
    setFullScreen(false);//设置窗口不全屏
...
void MainWindow::updateRecentFilesMenu()
{
    ...
    for (int i = 0; i < numRecentFiles; ++i) {
        ...
        mRecentFiles[i]->setData(files[i]);//Sets the action's internal data to the given userData.
        mRecentFiles[i]->setVisible(true);
    }
    for (int j = numRecentFiles; j < Preferences::MaxRecentFiles; ++j) {
        mRecentFiles[j]->setVisible(false);//如果没有到达最大值,其他几个动作都不显示
    }
    mUi->menuRecentFiles->setEnabled(numRecentFiles > 0);//如果没有最近文件该菜单项为失能
}
...
void MainWindow::exportMapAs(MapDocument *mapDocument)
{
    ...
    auto exportDetails = chooseExportDetails<MapFormat>(fileName,
                                                        ...
                                                        selectedFilter,
                                                        this);
    if (!exportDetails.isValid()) {//exportDetails != nullptr
        return;
    }
    ...
    if (outputFiles.size() > 0) {
        // Check if any output file already exists检查是否已经存在输出文件
        QString message =
