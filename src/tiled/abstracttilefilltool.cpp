...
AbstractTileFillTool::AbstractTileFillTool(const QString &name,
    ...
    , mWangSet(nullptr)
{
    connect(mStampActions->random(), &QAction::toggled/*触发器*/, this, &AbstractTileFillTool::randomChanged);
    connect(mStampActions->wangFill()/*wang填充模式*/, &QAction::toggled, this, &AbstractTileFillTool::wangFillChanged);
...
void AbstractTileFillTool::deactivate(MapScene *scene)//使什么无效
{
...
void AbstractTileFillTool::populateToolBar(QToolBar *toolBar)//填充工具栏
{//StampActions* mStampActions//object
...
void AbstractTileFillTool::randomFill(TileLayer &tileLayer, const QRegion &region) const//随机填充
{
...
void AbstractTileFillTool::fillWithStamp(Map &map,//填充图章
